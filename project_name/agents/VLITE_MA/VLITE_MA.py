...
class VLITE_MAAgent(AgentBase):
    def __init__(self,
        ...
        self.opp_rp_network = EnsembleOppNetwork(config=config, agent_config=self.agent_config)
    ...
    def create_train_state(self):
        def create_ensemble_state(key: chex.PRNGKey) -> TrainStateRP:  # TODO is this the best place to put it all?
            ...
            rp_params = self.rp_network.init(_key,
                                             ...
                                             jnp.zeros((1, self.config.NUM_ENVS, self.config.NUM_AGENTS - 1)),)["params"],
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _get_reward_noise(self, ens_state: TrainStateRP, obs: chex.Array, actions: chex.Array, opp_actions: chex.Array) -> chex.Array:
        ...
        ensemble_opp_actions = jnp.broadcast_to(opp_actions, (self.agent_config.NUM_ENSEMBLE, *actions.shape))
    ...
    def _reward_noise_over_actions(self, ens_state: TrainStateRP, obs: chex.Array) -> chex.Array:
        ...
        opp_actions = jnp.broadcast_to(jnp.expand_dims(opp_actions, axis=-1), (*opp_actions.shape, self.config.NUM_ENVS))
    ...
    def _entropy_loss_fn(self, logits_t_ego, logits_t_opp, uncertainty_t):
        ...
        return -jnp.sum(pi_times_log_pi_ego * pi_times_log_pi_opp * uncertainty_t, axis=-1)
    ...
    def update(self, runner_state, agent, traj_batch):
        ...
        state_action_reward_noise = self._get_reward_noise(train_state.ens_state, traj_batch.obs, traj_batch.action, action_opp)
        state_reward_noise = self._reward_noise_over_actions(train_state.ens_state, traj_batch.obs)

        def ac_loss(params, opp_params, trajectory, obs, state_action_reward_noise):
            ...
            # TODO how to pick one ensemble model as my like thompson sampling opp for an episode?
            _, opp_values, opp_logits = train_state.opp_state.apply_fn(opp_params, obs)
            opp_policy_dist = distrax.Categorical(logits=opp_logits[:-1])
            log_prob_opp = opp_policy_dist.log_prob(action_opp)
