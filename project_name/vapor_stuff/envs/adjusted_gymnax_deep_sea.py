...
@struct.dataclass
class InfoState:  # TODO added this info state for ease of metrics
    env_state: EnvState
...
class DeepSea(environment.Environment[EnvState, EnvParams]):
    ...
    def step_env(
        self,
        key: chex.PRNGKey,
        initial_state: InfoState,  # TODO changed this
        action: Union[int, float, chex.Array],
        params: EnvParams,
    ) -> Tuple[chex.Array, EnvState, jnp.ndarray, jnp.ndarray, Dict[Any, Any]]:
        """Perform single timestep state transition."""
        
        state = initial_state.env_state  # TODO added this
        ...
        new_episode_return = initial_state.episode_returns + reward
        new_episode_length = initial_state.episode_lengths + 1
        wrapper_state = InfoState(env_state=state,
                                  episode_returns=new_episode_return * (1 - done),
                                  episode_lengths=new_episode_length * (1 - done),
                                  returned_episode_returns=initial_state.returned_episode_returns * (
                                          1 - done) + new_episode_return * done,
                                  returned_episode_lengths=initial_state.returned_episode_lengths * (
                                          1 - done) + new_episode_length * done,
                                  )
        info["returned_episode_returns"] = wrapper_state.returned_episode_returns
        info["returned_episode_lengths"] = wrapper_state.returned_episode_lengths
        info["returned_episode"] = done
        ### to here

        return (
            lax.stop_gradient(self.get_obs(state)),
            lax.stop_gradient(wrapper_state),  # TODO changed this
            reward,
            done,
            info,
        )
    ...
    def reset_env(
        ...
        return self.get_obs(state), wrapper_state
